...
-- 'Expiration Date' : (FOR RENEWAL ONLY) original expiration date of parent Record or LP
 ...
 function licenseRenewalIssuance(itemCapId, recordSettings) {
	 ...
	 if (parentCapId) {
        ...
        //Renewal Status needs to be updated

        logDebug("UPDATING RENEWAL RECORD STATUS NOW");
        renewalCapProject = getRenewalCapByParentCapIDForReview(parentCapId);
        if (renewalCapProject != null)
        {
            //Set B1PERMIT.B1_ACCESS_BY_ACA to "N" for CAP to not allow that it is searched by ACA user.
            aa.cap.updateAccessByACA(capId, "N");

            //5. Set parent license to "Active"
            if (activeLicense(parentCapId))
            {
                //Set renewal CAP status to "Complete"
